...
private:
    string statistics;
public:
    void update(SubjectInterface* s) override {
...
public:
    void update(shared_ptr<SubjectInterface> s) override {
        shared_ptr<WeatherStation> ws =  s;
...
public:
    void update(shared_ptr<SubjectInterface> s) override {
        auto ws = shared_ptr<WeatherStation> s;
...
public:
    ...
    vector<shared_ptr<ObserverInterface>> getObserverList() {
        ...
        observers.push_back( (ObserverInterface*) &statistics_obs);
        observers.push_back( (ObserverInterface*) &current_obs);
        observers.push_back( (ObserverInterface*) &forecast_obs);
        return observers;
    }
};
